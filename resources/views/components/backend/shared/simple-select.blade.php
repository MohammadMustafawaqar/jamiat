...
<div class="{{ $col }}">
    <div class="form-group">
        ...
        @if($label!='default')<label for="{{$id}}" class="mb-0">{{ $label }} {!! $required==1 ? '<i
                class="text-danger">*</i>' : '' !!}</label>@endif
        <select {{ $attributes }} name="{{ $name }}" id="{{ $id }}"
            class="{{ $classes }} @error($name) is-invalid @enderror" @if($disabled) disabled @endif @if($readonly)
            readonly @endif>
